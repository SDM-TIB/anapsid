...
def createPlan(query, adaptive, buffersize, c):
    operatorTree = includePhysicalOperatorsQuery(query, adaptive, buffersize, c)
    # print(operatorTree.aux(""))
    return operatorTree


def includePhysicalOperatorsQuery(query, a, buffersize, c):
    return includePhysicalOperatorsUnionBlock(query, query.body, a, buffersize, c)


def includePhysicalOperatorsUnionBlock(query, ub, a, buffersize, c):
    r = []
    for jb in ub.triples:
        r.append(includePhysicalOperatorsJoinBlock(query, jb, a, buffersize, c))
    ...
    if isinstance(jb.triples, list):
        for bgp in jb.triples:
            if isinstance(bgp, Node) or isinstance(bgp, Leaf):
                tl.append(includePhysicalOperators(query, bgp, a, buffersize, c))
            elif isinstance(bgp, Optional):
                ol.append(includePhysicalOperatorsUnionBlock(query, bgp.bgg, a, buffersize, c))
            elif isinstance(bgp, UnionBlock):
                tl.append(includePhysicalOperatorsUnionBlock(query, bgp, a, buffersize, c))
...
def includePhysicalOperators(query, tree, a, buffersize, c):
    if isinstance(tree, Leaf):
        if isinstance(tree.service, Service):
            ...
            return includePhysicalOperatorsUnionBlock(query, tree.service, a, buffersize, c)
        else:
            print("PlanPIDS.py:260")

    elif isinstance(tree, Node):
        left_subtree = includePhysicalOperators(query, tree.left, a, buffersize, c)
        right_subtree = includePhysicalOperators(query, tree.right, a, buffersize, c)
